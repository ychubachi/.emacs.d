...
- leaf/straightからuse-package/elpacaに変更する
** この設定について
...
* 初期化 - パッケージマネージャ等を初期化してOrgを最新版にする
** elpaca/use-package - パッケージマネージャーの初期化
...
- Windowsではsymlinkを使わないようにする
#+begin_src emacs-lisp :tangle "init.el"
  (when (eq system-type 'windows-nt)
    (elpaca-no-symlink-mode))
#+end_src

- elpacaとuse-packageを連携させる
#+begin_src emacs-lisp :tangle "init.el"
...
** no-littering - Emacsが内部で作成するファイルの保存先の整理
#+begin_src emacs-lisp :tangle "init.el"
...
** COMMENT TODO ネイティブコンパイルのキャッシュ設定 -> early-init.elへ
+ no-litteringのための設定
...
*** 日本語入力
**** mozc
- WSLではWindowsのGoogle IMEを利用する
- WSL以外でのことはあとで考えよう
  - mozc_emacs_helper.sh を作ればよいか。
...
**** TODO mozc-cursor-color
GitHubから直接インストールして設定する
...
#+RESULTS:
: [nil 27264 13649 113156 nil elpaca-process-queues nil nil 0 nil]
...
**** COMMENT TODO isearch字にIMEの切り替えを有効にする →いらない？
#+begin_src emacs-lisp
  (use-package isearch
...
#+RESULTS:
: [nil 27265 30541 382546 nil elpaca-process-queues nil nil 0 nil]

** COMMENT Org関連パッケージの設定
**** 
...
#+begin_src emacs-lisp
  (use-package *org-use-speed-commands
    :config
...
**** ox-pandoc
#+begin_src emacs-lisp
  (use-package ox-pandoc :straight t :require t)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package list-utils :straight t)
  (use-package iedit :straight t)
  (use-package files+ :straight t)
  (use-package ls-lisp+ :straight t)
  (use-package w32-browser :straight t)
  (use-package dired+
...
** FIXME headline
#+begin_src emacs-lisp
        (use-package TODO:Move-To-Builtin
          :init
          #+end_src
** ffap
#+begin_src emacs-lisp
...
** nerd-icons - all-the-iconsより多くのアイコンを利用できるようにする
- インストールできない
...
**** coverage
#+begin_src emacs-lisp
  (use-package coverage :straight t)
#+end_src
**** dockerfile-mode
#+begin_src emacs-lisp
...
*** wanderlust
#+begin_src emacs-lisp
            (use-package wanderlust :straight t
              :config
              ;; IMAP
...
#+end_src
*** org-modeとimenuの連携 → use-packageにする
...
## Local Variables:
## eval: (add-hook 'after-save-hook (lambda () (when (y-or-n-p "Tangle init.org?") (org-babel-tangle))) nil t)
## End:
